...
const Experiencepage = () => {
  ...
  const fetchlistdata = async () => {
    try {
      const response = await axios.get(
        "http://localhost:1337/api/experiencelists"
      );
  ...
  return (
    <section className="bg-black text-white py-14 px-12 lg:py-28 lg:px-48 grid gap-10 ">
      <h1 data-aos="flip-up" className="text-4xl font-bold text-center">
        Experience
      </h1>
      <p data-aos="zoom-in-right" className="text-[#999999] text-center">
        😃Embrace the <span className="text-white">challenge</span>, for in
        facing difficult tasks and <span className="text-white">mastering</span>{" "}
        new technologies,{" "}
        <span className="text-white text-lg font-bold underline ">
          you unlock the door
        </span>{" "}
        to <span className="text-white">growth</span> and{" "}
        <span className="text-white">endless possibilities</span>🔥.
      </p>
      <div>
        <ul
          data-aos="fade-right"
          ...
          {experience.map((val) => (
            ...
            >
              ...
              <div className="grid gap-5 text-center ">
                <p className=" text-xl font-bold bg-gradient-to-r from-sky-400 to-sky-600 bg-clip-text text-transparent">
                  As a {val.attributes.role}
                </p>
                <p className="text-base font-medium  ]">
                  {val.attributes.Technologies}
                </p>
              </div>
              <div
                data-aos="fade-left"
              ...
              >
                <ul className="">
                  {experiencelist.map((val) => (
                    <li
                      key={val.id}
                      className="border rounded-xl border-[#1199EE] my-4 text-white p-5"
                    >
                      <p className="text-lg font-semibold text-white">
                        {val.attributes.experiencelist}
                      </p>
                    </li>
                  ))}
                </ul>
